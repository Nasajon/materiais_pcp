...
class GrupoDeRecursoDropdownWidget extends StatefulWidget {
  ...
  @override
  State<GrupoDeRecursoDropdownWidget> createState() =>
      _GrupoDeRecursoDropdownWidgetState();
}

class _GrupoDeRecursoDropdownWidgetState
    extends State<GrupoDeRecursoDropdownWidget> {
  final _store = Modular.get<GrupoDeRecursoListStore>();

  @override
  Widget build(BuildContext context) {
    return ScopedBuilder<GrupoDeRecursoListStore, Failure,
        List<GrupoDeRecurso>>(
      store: _store,
      onLoading: (context) =>
          DropdownLoadWidget(label: context.l10n.materiaisPcpGrupoDeRecurso),
      onState: (context, state) {
        if (state.isEmpty) {
          return DropdownLoadWidget(
        ...
        return CustomDropdownWidget(
          ...
          items: _store.state
              .map((grupoDeRecurso) => DropdownItem(
                  value: grupoDeRecurso, label: grupoDeRecurso.descricao))
              .toList(),
        );
      },
    );
