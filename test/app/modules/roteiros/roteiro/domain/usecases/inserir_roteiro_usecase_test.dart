...
void main() {
  ...
  late InserirRoteiroUsecase inserirRoteiroUsecase;

  setUp(() {
    roteiroRepository = RoteiroRepositoryMock();
    inserirRoteiroUsecase = InserirRoteiroUsecaseImpl(roteiroRepository);
  });

  group('InserirRoteiroUsecaseImpl -', () {
    group('Sucesso -', () {
      test('Deve inserir um roteiro quando os dados forem valido.', () async {
        when(() => roteiroRepository.inserirRoteiro(roteiro)).thenAnswer((_) async => '1');

        final response = await inserirRoteiroUsecase(roteiro);
    ...
    group('Falha -', () {
      test('Deve retornar o erro RoteiroIsNotValidFailure quando os dados não são valido.', () async {
        expect(() => inserirRoteiroUsecase(RoteiroAggregate.empty()), throwsA(isA<RoteiroIsNotValidFailure>()));
      });

      test('Deve retornar uma falha quando ocorrer um erro no repository.', () async {
        when(() => roteiroRepository.inserirRoteiro(roteiro)).thenThrow(RoteiroFailure());

        expect(() => inserirRoteiroUsecase(roteiro), throwsA(isA<Failure>()));
      });
    });
  });
}
