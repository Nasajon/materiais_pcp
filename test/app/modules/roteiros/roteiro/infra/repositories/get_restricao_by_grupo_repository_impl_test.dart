    ...
    group('remote -', () {
      group('sucesso -', () {
        test('Deve retornar uma lista das restrições quando informar o id do grupo de restrição.', () async {
          when(() => remoteGetRestricaoByGrupoDatasource('1')).thenAnswer((_) async => <RestricaoAggregate>[]);
      ...
      group('falha -', () {
        test('Deve retornar um RemoteDatasourceRoteiroFailure quando ocorrer erro mapeado no Datasource.', () async {
          ...
          expect(() => getRestricaoByGrupoRepository(''), throwsA(isA<RemoteDatasourceRoteiroFailure>()));
        });
      });
    });
  });
